...
class GiveawayView(discord.ui.View):
    ...
    async def enter(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        giveaway = await self.cog.bot.db.fetchone(
            "SELECT * FROM giveaways WHERE message_id=? AND ended_at IS NULL", (message_id,)
        )
        if not giveaway:
        ...
        if entry:
            ...
            await self.cog.bot.db.execute(
                ...
                (giveaway["id"], interaction.user.id, discord.utils.utcnow().isoformat()),
            )
            message = "You are entered. Good luck!"
        await interaction.response.send_message(message, ephemeral=True)


class Giveaways(ModuleCog):
    giveaway = app_commands.Group(
        name="giveaway",
        description="Button giveaways that survive restarts",
        guild_only=True,
        default_permissions=discord.Permissions(manage_guild=True),
    )
    module_name = "giveaways"
    ...
    async def _finish(self, giveaway_id_payload: dict) -> None:
        ...
        await self.bot.db.execute(
            "UPDATE giveaways SET ended_at=?, winner_ids=? WHERE id=?",
            (discord.utils.utcnow().isoformat(), json.dumps(winners), giveaway_id),
        )
        ...
        try:
            await channel.send(
                embed=embed(
                    "Giveaway ended",
                    f"**Prize:** {giveaway['prize']}\n**Winner{'s' if len(winners) != 1 else ''}:** {mention}",
    ...
    @giveaway.command(name="create", description="Start a button-based giveaway")
    async def gcreate(
        self,
        ...
        if not isinstance(interaction.channel, discord.TextChannel):
            await interaction.response.send_message("Start giveaways in a text channel.", ephemeral=True)
            return
        try:
        ...
        giveaway_id = await self.bot.db.execute(
            "INSERT INTO giveaways (guild_id,channel_id,host_id,prize,winners,required_role_id,ends_at) VALUES (?,?,?,?,?,?,?)",
            (
        ...
        message = await interaction.channel.send(
            embed=embed(
                ...
                f"{f'**Required role:** {required_role.mention}' if required_role else ''}",
                colour=discord.Colour(0xFEE75C),
            ),
            view=view,
        )
    ...
    async def gend(self, interaction: discord.Interaction, giveaway_id: int) -> None:
        row = await self.bot.db.fetchone(
            "SELECT * FROM giveaways WHERE id=? AND guild_id=? AND ended_at IS NULL",
        ...
        await interaction.response.send_message(
            embed=embed("End giveaway?", f"This immediately selects winners for giveaway **#{giveaway_id}**."),
            view=confirmation,
            ephemeral=True,
        )
    ...
    @giveaway.command(name="reroll", description="Choose a fresh winner for an ended giveaway")
    async def greroll(self, interaction: discord.Interaction, giveaway_id: int) -> None:
        row = await self.bot.db.fetchone(
            "SELECT * FROM giveaways WHERE id=? AND guild_id=? AND ended_at IS NOT NULL",
            (giveaway_id, interaction.guild_id),
        )
        if not row:
            ...
            return
        entries = await self.bot.db.fetchall("SELECT user_id FROM giveaway_entries WHERE giveaway_id=?", (giveaway_id,))
        old = set(json.loads(row["winner_ids"] or "[]"))
    ...
    async def glist(self, interaction: discord.Interaction) -> None:
        rows = await self.bot.db.fetchall(
            "SELECT * FROM giveaways WHERE guild_id=? AND ended_at IS NULL ORDER BY ends_at",
            (interaction.guild_id,),
        )
        description = "\n".join(
            f"**#{row['id']}** {row['prize']} • ends in "
            f"{human_duration(max(0, int((parse_iso(row['ends_at']) - utcnow()).total_seconds())))} • <#{row['channel_id']}>"
            for row in rows
        )
