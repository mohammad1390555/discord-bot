...
class TicketTopicSelect(discord.ui.Select):
    def __init__(self, cog: "Tickets") -> None:
        self.cog = cog
        super().__init__(placeholder="Choose a ticket topic", custom_id="aegis:ticket:topic", options=[
...
class Tickets(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot
        # Persistent ticket controls continue working after a restart.
    ...
    async def open_ticket(self, interaction: discord.Interaction, topic: str) -> None:
        ...
        if guild is None:
            return
        existing = await self.bot.db.fetchone("SELECT * FROM tickets WHERE guild_id=? AND opener_id=? AND closed_at IS NULL", (guild.id, interaction.user.id))
        if existing:
            channel = guild.get_channel(existing["channel_id"])
            await interaction.response.send_message(f"You already have an open ticket: {channel.mention if channel else 'ticket #' + str(existing['id'])}", ephemeral=True)
            return
        ...
        if support:
            overwrites[support] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_messages=True)
        channel = await guild.create_text_channel(f"ticket-{interaction.user.name}"[:90], category=category if isinstance(category, discord.CategoryChannel) else None, overwrites=overwrites, reason=f"Ticket opened by {interaction.user}")
        ticket_id = await self.bot.db.execute("INSERT INTO tickets (guild_id,channel_id,opener_id,topic,opened_at) VALUES (?,?,?,?,?)", (guild.id, channel.id, interaction.user.id, topic, discord.utils.utcnow().isoformat()))
        await channel.send(content=f"{interaction.user.mention} {support.mention if support else ''}", embed=embed(f"Ticket #{ticket_id} — {topic}", "A member of the support team will be with you shortly. Use `/ticket close` when resolved.", colour=discord.Colour(0x57F287)))
    ...
    async def ticket_setup(self, interaction: discord.Interaction, category: discord.CategoryChannel | None = None, support_role: discord.Role | None = None) -> None:
        await self.bot.db.set_settings(interaction.guild_id, **{"ticket.category_id": category.id if category else None, "ticket.support_role_id": support_role.id if support_role else None})
        message = await interaction.channel.send(embed=embed("Need help?", "Choose a topic below to open a private ticket."), view=TicketPanelView(self))
        if message:
    ...
    async def ticket_close(self, interaction: discord.Interaction) -> None:
        ...
        if log_channel and hasattr(log_channel, "send"):
            await log_channel.send(content=f"Ticket #{row['id']} closed by {interaction.user}.", file=discord.File(transcript, filename=f"ticket-{row['id']}.txt"))
        await interaction.followup.send(embed=ok("Ticket closed. This channel will be deleted in 10 seconds."))
