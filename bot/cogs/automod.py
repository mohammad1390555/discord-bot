...
class AutoModeration(commands.Cog):
    """Configurable message guard and lightweight anti-raid protection."""
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        if role and role < guild.me.top_role:  # type: ignore[union-attr]
            try:
        ...
        if channel and hasattr(channel, "send"):
            ...
            text = template.format(user=member.mention, server=guild.name, membercount=guild.member_count)
            try:
    ...
    async def on_member_remove(self, member: discord.Member) -> None:
        ...
        if channel and hasattr(channel, "send"):
            ...
            try:
                await channel.send(embed=embed("Member left", template.format(user=str(member), server=member.guild.name, membercount=member.guild.member_count)))
            except discord.HTTPException:
                pass
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if previous and previous[0] == content and discord.utils.utcnow().timestamp() - previous[1] < 15:
            violations.append("duplicate message spam")
        self.recent[key] = (content, discord.utils.utcnow().timestamp())
        if not violations:
            return
        try:
            ...
            pass
        await self.bot.db.add_warning(message.guild.id, message.author.id, self.bot.user.id, "AutoMod: " + ", ".join(violations))  # type: ignore[union-attr]
        ...
        try:
            ...
            pass
        try:
            await message.channel.send(embed=embed("Message removed", f"{message.author.mention}, your message was removed for {', '.join(violations)}.", colour=DANGER), delete_after=8)
        except discord.HTTPException:
            pass
