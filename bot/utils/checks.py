...
def can_moderate(member: discord.Member, target: discord.Member) -> None:
    ...
    if target.top_role >= member.top_role and member.id != member.guild.owner_id:
        raise commands.BadArgument("That member has an equal or higher role than you.")


def can_bot_moderate(guild: discord.Guild, target: discord.Member) -> None:
    me = guild.me
    if me and target.top_role >= me.top_role:
        raise commands.BadArgument("That member has an equal or higher role than the bot.")
...
async def send_ephemeral(interaction: discord.Interaction, message: str, *, colour=0xED4245) -> None:
