...
class PollView(discord.ui.View):
    def __init__(self, question: str, options: list[str]) -> None:
        ...
        for index, option in enumerate(options):
            button = discord.ui.Button(label=shorten(option, 75), style=discord.ButtonStyle.primary, custom_id=f"poll:{id(self)}:{index}")
    ...
    def result_embed(self, counts: list[int] | None = None) -> discord.Embed:
        ...
        total = sum(counts)
        body = "\n".join(f"**{i + 1}. {option}** — {count} vote{'s' if count != 1 else ''}" for i, (option, count) in enumerate(zip(self.options, counts)))
...
class Utility(commands.Cog):
    ...
    async def _deliver_reminder(self, payload: dict) -> None:
        ...
        try:
            await destination.send(embed=embed("Reminder", reminder["message"]))
    ...
    async def help_command(self, interaction: discord.Interaction) -> None:
        pages = {
            "moderation": embed("Moderation", "`/ban` `/kick` `/warn` `/warnings` `/mutetimeout` `/purgeclear` `/lockunlock` `/slowmode`\n\nStaff commands require the matching Discord permission."),
            "server": embed("Server configuration", "`/setup` `/settings` `/setprefix` `/setlogchannel` `/setwelcome` `/setleave` `/autorole` `/exportconfig`"),
            "utility": embed("Utility", "`/userinfo` `/serverinfo` `/avatar` `/banner` `/ping` `/botinfostats` `/remindme` `/poll` `/afk` `/translate` `/weather` `/define` `/qr` `/shorten` `/snipe`"),
            "community": embed("Community", "`/ranklevel` `/leaderboard` `/balance` `/daily` `/weekly` `/work` `/pay` `/shop` `/inventory`\n`/8ball` `/joke` `/fact` `/quote` `/rps` `/ship` `/rate`"),
            "support": embed("Tickets and giveaways", "`/ticket setup` `/ticket close` `/ticket claim` `/ticket add` `/ticket remove`\n`/gcreate` `/gend` `/greroll` `/glist`"),
        }
    ...
    async def ping(self, interaction: discord.Interaction) -> None:
        ...
        await message.edit(embed=info(f"Gateway: **{self.bot.latency * 1000:.0f} ms**\nAPI: **{(discord.utils.utcnow() - started).total_seconds() * 1000:.0f} ms**"))
    ...
    async def roleinfo(self, interaction: discord.Interaction, role: discord.Role) -> None:
        ...
        result.add_field(name="Permissions", value=shorten(", ".join(p.replace("_", " ") for p, enabled in role.permissions if enabled) or "None"))
    ...
    @app_commands.command(name="botinfostats", description="Show Aegis uptime and resource statistics")
    async def botinfostats(self, interaction: discord.Interaction) -> None:
        guilds = len(self.bot.guilds)
        users = sum(guild.member_count or 0 for guild in self.bot.guilds)
        result = embed(f"{self.bot.settings.bot_name} statistics", f"Uptime: **{human_duration(self.bot.uptime_seconds)}**\nPython: **{__import__('platform').python_version()}**\ndiscord.py: **{discord.__version__}**\nMemory: **{self.bot.memory_mb:.1f} MB**")
    ...
    async def remindme(self, interaction: discord.Interaction, duration: str, message: str, dm: bool = False) -> None:
        ...
        reminder_id = await self.bot.db.create_reminder(None if dm else interaction.guild_id, interaction.user.id, None if dm else interaction.channel_id, message[:1000], run_at)
        await interaction.response.send_message(embed=ok(f"Reminder **#{reminder_id}** set for <t:{int(run_at.timestamp())}:R>."), ephemeral=True)
    ...
    async def afk(self, interaction: discord.Interaction, reason: str = "AFK") -> None:
        if interaction.guild_id is None:
            ...
            return
        await self.bot.db.execute("INSERT INTO afk (guild_id,user_id,reason,created_at) VALUES (?,?,?,?) ON CONFLICT(guild_id,user_id) DO UPDATE SET reason=excluded.reason, created_at=excluded.created_at", (interaction.guild_id, interaction.user.id, reason[:250], discord.utils.utcnow().isoformat()))
        await interaction.response.send_message(embed=ok(f"AFK set: **{reason[:250]}**"), ephemeral=True)

    async def on_message(self, message: discord.Message) -> None:
        if not message.guild or message.author.bot:
            return
        mentioned = await self.bot.db.fetchone("SELECT * FROM afk WHERE guild_id=? AND user_id=?", (message.guild.id, message.author.id))
        if mentioned:
            await self.bot.db.execute("DELETE FROM afk WHERE guild_id=? AND user_id=?", (message.guild.id, message.author.id))
            try:
                await message.channel.send(embed=info(f"Welcome back, {message.author.mention}. Your AFK status was cleared."), delete_after=8)
            except discord.HTTPException:
                pass
        for user in message.mentions:
            afk = await self.bot.db.fetchone("SELECT * FROM afk WHERE guild_id=? AND user_id=?", (message.guild.id, user.id))
            if afk:
    ...
    async def translate(self, interaction: discord.Interaction, text: str, target: str = "en") -> None:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json={"q": text[:3000], "source": "auto", "target": target.lower()}) as response:
    ...
    async def weather(self, interaction: discord.Interaction, city: str) -> None:
        ...
        if not key:
            await interaction.response.send_message("Weather is disabled until OPENWEATHER_API_KEY is configured.", ephemeral=True)
            return
        await interaction.response.defer()
        ...
        try:
            ...
            return
        await interaction.followup.send(embed=embed(f"Weather — {data['name']}", f"**{data['weather'][0]['description'].title()}**\nTemperature: **{data['main']['temp']:.1f}°C**\nFeels like: **{data['main']['feels_like']:.1f}°C**\nHumidity: **{data['main']['humidity']}%**"))
    ...
    async def define(self, interaction: discord.Interaction, word: str) -> None:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https://api.dictionaryapi.dev/api/v2/entries/en/{urllib.parse.quote(word)}") as response:
                    data = await response.json()
            ...
            return
        await interaction.followup.send(embed=embed(f"Definition — {word}", definition + (f"\n\n*Example: {example}*" if example else "")))
    ...
    async def shorten_url(self, interaction: discord.Interaction, url: str) -> None:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://tinyurl.com/api-create.php", params={"url": url}) as response:
                    shortened = await response.text()
            if not shortened.startswith("http"):
                raise ValueError
        except (aiohttp.ClientError, ValueError):
            await interaction.response.send_message(embed=error("The URL shortener is unavailable."), ephemeral=True)
            return
        await interaction.response.send_message(embed=ok(f"Short URL: {shortened}"))
    ...
    async def snipe(self, interaction: discord.Interaction) -> None:
        ...
        if not data:
            ...
            return
        await interaction.response.send_message(embed=embed("Last deleted message", f"**{data['author']}**\n{data['content'] or '*no text*'}"))
    ...
    async def editsnipe(self, interaction: discord.Interaction) -> None:
        ...
        if not data:
            ...
            return
        await interaction.response.send_message(embed=embed("Last edited message", f"**{data['author']}**\n**Before:** {data['before']}\n**After:** {data['after']}"))
